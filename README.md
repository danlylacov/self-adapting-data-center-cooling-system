...
### Качество отдельных ML-моделей (offline)

Метрики ниже относятся к **обучению/валидации** сервисов на исторических данных и **не равны** напрямую «проценту выигрыша» в оркестраторе, но показывают качество прогнозов.

**Прогноз нагрузки** (`predict_load`, почасовой ряд, см. [`models/predict_load/README.md`](models/predict_load/README.md)):

| Модель | Выборка | MAE | RMSE | MAPE |
|--------|---------|-----|------|------|
| Prophet (`prophet_full48`) | val | 1.064 | 1.317 | 5.51% |
| Prophet | test | 1.233 | 1.544 | 6.15% |
| DeepAR (`deepar_serving`) | val | 1.113 | 1.327 | 5.47% |
| DeepAR | test | 1.348 | 1.749 | 6.79% |

На текущем состоянии репозитория на **test** чуть лучше **Prophet** (см. вывод в README модуля).

**Гибридный PUE** обучается предсказывать **остаток** `residual = pue_real − pue_physics`; качество удобно смотреть по ошибке residual и по итоговому PUE на отложенной выборке (см. [`models/predict_pue/README.md`](models/predict_pue/README.md)).

**Реализм траекторий двойника** (не энергия, а «правдоподобность» температур и PUE): скрипт [`DC_digital_twin/scripts/benchmark_realism.py`](DC_digital_twin/scripts/benchmark_realism.py) выставляет балл **0…100** по множеству прогонов и сохраняет `results/realism_benchmark.csv`.

### ML vs «простой прогон» vs генетический алгоритм

**Универсальной константы вида «ML эффективнее на X%» в репозитории нет:** средний PUE, риск перегрева и энергия охлаждения зависят от **числа шагов**, **`deltaTime`**, **нагрузки**, **погоды**, **интервала управления** и доступности ML. Сравнение имеет смысл только при **одинаковом сценарии** и одинаковой настройке реализма.

| Режим | Где запускается | Что использует | Смысл |
|--------|-----------------|----------------|--------|
| **Простой прогон** | Страница «Прогон», прямой вызов API двойника | Только симулятор; **уставка и вентиляторы CRAC не перенастраиваются** по ходу сценария (остаются как заданы) | Базовая линия «без замкнутого умного управления» |
| **ML-оркестратор** | `POST /run`, страница «ML-прогон» | `predict_load` + `predict_pue` + `predict_server_temp`, правила **safety** и опционально **temp-aware PUE** | Учитывает **прогноз нагрузки**, **гибридный PUE** и **риск перегрева** по LSTM; может снижать PUE за счёт сдвига уставки, но требует работающих сервисов |
| **GA-оркестратор** | `POST /run/ga`, страница «GA-прогон» | Только **`predict_ga`** (политика из `GA/tuned_params.json`) | Не вызывает Prophet/PUE/temp ML; политика обучена **офлайн** генетическим алгоритмом на сценарии, близком к обучению — на других условиях может быть лучше или хуже ML |

**Когда что может выглядеть «выгоднее»:**

- **ML** потенциально выигрывает там, где важны **горизонт** нагрузки, **гибридный PUE** и **температурный safety** с разными источниками сигналов.
- **GA** проще по цепочке зависимостей (один лёгкий сервис + двойник), стабилен при отключённых ML-прогнозах, но **не использует** те же мультимодельные прогнозы.
- **Простой прогон** обычно даёт **менее гибкое** поведение по энергии/температуре, зато однозначно воспроизводимый baseline.

Обучение GA фиксирует **фитнес** по сценарию (пример: `fitness_best` ≈ `1.99·10⁵` в [`GA/experiments/exp01_baseline.json`](GA/experiments/exp01_baseline.json)); это **внутренняя цель эволюции**, её нельзя напрямую сравнивать с PUE из UI без той же постановки задачи.

### Как получить численное сравнение на своём сценарии

1. Поднять двойник и оркестратор (и нужные ML-сервисы).
2. Запустить скрипт сравнения трёх режимов на **одинаковых** параметрах (по умолчанию 120 шагов, `deltaTime=10`, seed 42):

   ```bash
   python scripts/compare_control_modes.py
   ```

   Переменные: `TWIN_BASE`, `ORCH_BASE`.

3. Дополнительно для **только ML**-сценариев с разными гиперпараметрами оркестратора:

   ```bash
   PYTHONPATH=. python orchestrator/scripts/benchmark_ml_runs.py
   ```

   Скрипт печатает средний/макс PUE, температуры, риск, ход уставки/вентилятора по нескольким пресетам `POST /run`.

Сравнивайте в первую очередь **средний PUE** по траектории и **финальный риск перегрева**; при равном риске ниже PUE в этой модели соответствует меньшей доле энергии инфраструктуры относительно IT (см. определение PUE в телеметрии двойника).
