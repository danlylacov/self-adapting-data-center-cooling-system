...
export default function OrchestratorGAPage() {
  ...
  const aggregates = useMemo(() => {
    ...
    const extra = []
    if (runMeta?.gaFallbackUsed) {
      extra.push(['GA fallback', 'да'])
    }
    if (runMeta?.gaSetpointRecommended != null) {
      extra.push(['Последняя уставка GA', `${Number(runMeta.gaSetpointRecommended).toFixed(2)} °C`])
    }
    if (runMeta?.gaFanRecommended != null) {
      extra.push(['Последние вентиляторы GA', `${Number(runMeta.gaFanRecommended).toFixed(1)} %`])
    }
    return [
  ...
  const runGaScenario = async () => {
    ...
    try {
      ...
      let seedVal = null
      if (runSeed !== '' && runSeed != null) {
        const n = Number(runSeed)
        if (Number.isFinite(n)) seedVal = Math.floor(n)
      }
      ...
      const body = {
        ...
        seed: seedVal,
        failOnGaUnavailable: false,
      }

      const orch = orchestratorBase()
      const { data } = await axios.post(`${orch}/run/ga`, body, { timeout: 600_000 })
      ...
      if (msg === 'Network Error' || e?.code === 'ERR_NETWORK') {
        setError(
          `Не удалось связаться с оркестратором (${orch}). Запустите оркестратор и сервис predict_ga (порт 8013): ` +
            `PYTHONPATH=. uvicorn orchestrator.app:app --port 8030; PYTHONPATH=. uvicorn app:app --port 8013 (из models/predict_ga).`,
        )
      } else {
  ...
  return (
    <div className="layout">
      <header className="topbar">
        <h1>GA-прогон (оркестратор)</h1>
        <p className="hint">
          Прогон через оркестратор ({orchestratorBase()}): на тиках управления вызывается сервис <strong>predict_ga</strong> (обученная
          политика из <code>tuned_params.json</code>). Без ML прогноза нагрузки, PUE и temp-модели.
        </p>
      </header>
      ...
      <section className="quickPanel">
        ...
        <div className="controlGrid">
          ...
          <label>
            <span className="labelRow">Seed симуляции (пусто = случайно)</span>
            <input type="number" value={runSeed} onChange={(e) => setRunSeed(e.target.value)} placeholder="например 42" />
          </label>
          <label>
            <span className="labelRow">
              Режим охлаждения
        ...
        <div className="actions">
          <button onClick={runGaScenario} disabled={progress.running}>
            Запустить GA-прогон
          </button>
            ...
            <div className="chartCard">
              ...
              <ResponsiveContainer width="100%" height={240}>
                <LineChart data={chartData}>
                  ...
                  <Tooltip />
                  <Line yAxisId="sp" type="monotone" dataKey="appliedSetpoint" stroke="#f5a623" dot={false} name="Setpoint (факт), °C" />
                  <Line yAxisId="sp" type="monotone" dataKey="gaSetpointRecommended" stroke="#c9a227" dot={false} strokeDasharray="4 4" name="Setpoint GA, °C" />
