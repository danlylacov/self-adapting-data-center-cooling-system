...
/** База URL оркестратора: в dev по умолчанию прокси Vite (/orchestrator → :8030), см. vite.config.js */
function orchestratorBase() {
  const raw = import.meta.env.VITE_ORCHESTRATOR_BASE
...
function formatApiError(e) {
  ...
  if (Array.isArray(d)) {
    return d.map((x) => (typeof x === 'object' && x !== null ? JSON.stringify(x) : String(x))).join('; ')
  }
...
export default function OrchestratorPage() {
  ...
  const [controlIntervalSteps, setControlIntervalSteps] = useState(5)
  const [safetyMaxPOverheat, setSafetyMaxPOverheat] = useState(0.2)
  const [tempAwarePue, setTempAwarePue] = useState(true)
  const [chipTempTargetC, setChipTempTargetC] = useState(62)
  const [chipTempDeadbandC, setChipTempDeadbandC] = useState(3)
  const [loadForecastModel, setLoadForecastModel] = useState('prophet')
  ...
  const twinRequest = async (method, path, data) => {
    const response = await axios({ method, url: `${API_BASE}${path}`, data })
    return response.data
  }

  const pushRealismToApi = async (payload) => {
    await twinRequest('post', '/realism/mode', { mode: payload.mode })
    return twinRequest('post', '/realism/params', {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const r = await twinRequest('get', '/realism')
  ...
  const aggregates = useMemo(() => {
    ...
    const extra = []
    if (runMeta?.mlFallbackUsed) {
      extra.push(['ML fallback', 'да'])
    }
    if (runMeta?.tempAwarePue === false) {
      extra.push(['Temp-aware PUE', 'выкл'])
    }
    if (runMeta?.pueDeltaRaw != null && runMeta?.pueDeltaApplied != null) {
      extra.push(['PUE delta (raw → applied)', `${Number(runMeta.pueDeltaRaw).toFixed(2)} → ${Number(runMeta.pueDeltaApplied).toFixed(2)} °C`])
    }
    return [
  ...
  const runMlScenario = async () => {
    ...
    try {
      const ds = await twinRequest('get', '/datasets')
      setDatasets(ds.datasets ?? [])

      const body = {
        scenario: {
          steps: scenario.steps,
          deltaTime: clamp(scenario.deltaTime, DELTA_TIME_MIN, DELTA_TIME_MAX),
          setpoint: scenario.setpoint,
          fanSpeed: scenario.fanSpeed,
          coolingMode: scenario.coolingMode,
          meanLoad: scenario.meanLoad,
          stdLoad: scenario.stdLoad,
          outsideTemp: scenario.outsideTemp,
          useDatasetLoad: scenario.useDatasetLoad,
          loadDatasetPath: scenario.useDatasetLoad ? selectedLoadDataset || null : null,
        },
        realism,
        controlIntervalSteps,
        safetyMaxPOverheat,
        loadForecastModel,
        tempAwarePue,
        chipTempTargetC,
      ...
      const { data } = await axios.post(`${orch}/run`, body, { timeout: 600_000 })
      const points = data.points ?? []
      setRunData(points)
      setFinalState(data.finalState ?? null)
      setRunMeta(data.meta ?? null)
      setProgress({ step: scenario.steps, total: scenario.steps, running: false })
    } catch (e) {
      const orch = orchestratorBase()
      const msg = e?.message
      if (msg === 'Network Error' || e?.code === 'ERR_NETWORK') {
        setError(
          `Не удалось связаться с оркестратором (${orch}). Запустите: ` +
            `PYTHONPATH=. uvicorn orchestrator.app:app --host 127.0.0.1 --port 8030 ` +
            `(из корня репозитория v2). В режиме npm run dev запросы идут на прокси /orchestrator → :8030.`,
        )
      } else {
        const code = e?.response?.status
        const prefix = code ? `[${code}] ` : ''
        setError(prefix + formatApiError(e))
      }
    } finally {
  ...
  return (
    <div className="layout">
      <header className="topbar">
        <h1>ML-прогон (оркестратор)</h1>
        <p className="hint">
          Прогон через сервис оркестратора ({orchestratorBase()}): PUE, прогноз нагрузки, риск перегрева по серверам.
        </p>
      </header>
      ...
      <section className="quickPanel">
        ...
        <div className="controlGrid">
          ...
          <label>
            <span className="labelRow">Интервал управления (шагов симуляции)</span>
            <input
              type="number"
              min={1}
              value={controlIntervalSteps}
              onChange={(e) => setControlIntervalSteps(Math.max(1, Number(e.target.value)))}
            />
          </label>
          <label>
            <span className="labelRow">Порог p_overheat (безопасность)</span>
            <input
              type="number"
              step="0.05"
              min={0}
              max={1}
              value={safetyMaxPOverheat}
              onChange={(e) => setSafetyMaxPOverheat(Number(e.target.value))}
            />
          </label>
          <label>
            <span className="labelRow">Temp-aware PUE (коридор по t_chip)</span>
            <input type="checkbox" checked={tempAwarePue} onChange={(e) => setTempAwarePue(e.target.checked)} />
          </label>
          <label>
            <span className="labelRow">Цель t_chip (°C)</span>
            <input
              type="number"
              min={30}
              max={95}
              step={0.5}
              value={chipTempTargetC}
              onChange={(e) => setChipTempTargetC(Number(e.target.value))}
            />
          </label>
          <label>
            <span className="labelRow">Мёртвая зона ± (°C)</span>
            <input
              type="number"
              min={0.1}
              max={20}
              step={0.5}
              value={chipTempDeadbandC}
              onChange={(e) => setChipTempDeadbandC(Number(e.target.value))}
            />
          </label>
          <label>
            <span className="labelRow">Модель прогноза нагрузки</span>
            <select value={loadForecastModel} onChange={(e) => setLoadForecastModel(e.target.value)}>
        ...
        <div className="actions">
          <button onClick={runMlScenario} disabled={progress.running}>
            Запустить ML-прогон
          </button>
        ...
        <div className="progressWrap">
          ...
          <div className="hint">
            Безопасный диапазон delta time: {DELTA_TIME_MIN}-{DELTA_TIME_MAX} сек · оркестратор: {orchestratorBase()}
          </div>
        </div>
      </section>
              ...
              <ResponsiveContainer width="100%" height={240}>
                <LineChart data={chartData}>
                  ...
                  <Line
                    ...
                    dot={false}
                    strokeWidth={1.2}
                    name="Средняя темп. чипа"
                  />
                  <Line yAxisId="left" type="monotone" dataKey="avgChipMa" stroke="#ffd2b0" dot={false} strokeWidth={2} name="Чип (сглаж.)" />
                  <Line yAxisId="left" type="monotone" dataKey="room" stroke="#4d8bff" dot={false} name="Темп. зала" />
                  <Line yAxisId="left" type="monotone" dataKey="roomMa" stroke="#bcd6ff" dot={false} name="Зал (сглаж.)" />
                  <Line yAxisId="left" type="monotone" dataKey="outside" stroke="#9ec5ff" dot={false} name="Наружная темп." />
                </LineChart>
              </ResponsiveContainer>
            </div>
            <div className="chartCard">
              <h3>Уставка и вентиляторы (применённые)</h3>
              <ResponsiveContainer width="100%" height={240}>
                <LineChart data={chartData}>
                  <CartesianGrid stroke="#2b3a5c" strokeDasharray="3 3" />
                  <XAxis dataKey="timeMin" />
                  <YAxis yAxisId="sp" domain={[18, 27]} label={{ value: '°C', angle: -90, position: 'insideLeft' }} />
                  <YAxis yAxisId="fan" orientation="right" domain={[0, 100]} label={{ value: '%', angle: 90, position: 'insideRight' }} />
                  <Tooltip />
                  <Line yAxisId="sp" type="monotone" dataKey="appliedSetpoint" stroke="#f5a623" dot={false} name="Setpoint, °C" />
                  <Line yAxisId="fan" type="monotone" dataKey="appliedFanSpeed" stroke="#7ed321" dot={false} name="Вентиляторы, %" />
                </LineChart>
              </ResponsiveContainer>
            </div>
            <div className="chartCard">
